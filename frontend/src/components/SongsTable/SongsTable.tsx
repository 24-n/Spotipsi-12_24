...
function SongsTable({ songs, favorites, playlists, setFavorites, setPlaylists }: Props) {
    const [anchorEl, setAnchorEl] = useState<HTMLButtonElement | null>(null);
    const handleClick = (event: React.MouseEvent<HTMLButtonElement>, CurrentPlayList?: string | undefined) => {
        playlists.length > 0 && setAnchorEl(event.currentTarget);
    ...
    const createPlaylistsElements = (songID: string) => {
        return playlists.map(playlist => {
            return (
                <MenuItem color="secondary" key={playlist.id + songID} onClick={() => updateServer(songID, undefined, playlist.id)}>{playlist.name}</MenuItem>
            );
        })
    }
